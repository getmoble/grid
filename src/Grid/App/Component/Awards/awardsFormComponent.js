ko.components.register('awardupdate', {
    viewModel: function (params) {
        ...
        self.setValue(params.entity, params.entity + "-form-template", new Awards({}), "create")

        self.save = function () {
            self.isBusy(true);
            var data = ko.toJS(self.selectedData);
            if (self.selectedData().modelState.isValid()) {
                var result = rapid.dataManager.postData(urls.awards.update, data);
                result.done(function (response) {
                    self.isBusy(false);
                    if (data.id == 0) {
                        self.selectedData(new Awards({}));
                        rapid.notificationManager.showSuccess(rapid.messageManager.createMessage("Award"));
                        rapid.eventManager.publish(self.entity + "Created");
                        rapid.eventManager.publish(self.entity + "Cancel");
                    }
                    else {
                        rapid.notificationManager.showSuccess(rapid.messageManager.updateMessage("Award"));
        ...
        amplify.subscribe(self.entity + "Update", function (data) {
            ...
            if (data.entityId) {
                var result = rapid.dataManager.getData(urls.awards.get + data.entityId);
                result.done(function (response) {
                    self.isBusy(false);
                    self.selectedData(new Awards(response.Result));
                });
            }
        });
    ...
    template: {
        fromUrl: 'Form/FormContent?type=Award'
    }
});