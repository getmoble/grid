ko.components.register('leadstatusupdate', {
    viewModel: function (params) {
        ...
        self.setValue(params.entity, params.entity + "-form-template", new LeadStatus({}), "create")

        self.save = function () {
            self.isBusy(true);
            var data = ko.toJS(self.selectedData);
            if (self.selectedData().modelState.isValid()) {
                var result = rapid.dataManager.postData(urls.leadstatus.update, data);
                result.done(function (response) {
                    self.isBusy(false);
                    if (data.id == 0) {
                        self.selectedData(new LeadStatus({}));
                        rapid.notificationManager.showSuccess(rapid.messageManager.createMessage("Lead Status"));
                        rapid.eventManager.publish(self.entity + "Created");
                        rapid.eventManager.publish(self.entity + "Cancel");
                    }
                    else {
                        rapid.notificationManager.showSuccess(rapid.messageManager.updateMessage("Lead Status"));
        ...
        amplify.subscribe(self.entity + "Update", function (data) {
            ...
            if (data.entityId) {
                var result = rapid.dataManager.getData(urls.leadstatus.get + data.entityId);
                result.done(function (response) {
                    self.isBusy(false);
                    self.selectedData(new LeadStatus(response.Result));
                });
            }
        });
    ...
    template: {
        fromUrl: 'Form/FormContent?type=LeadStatus'
    }
});